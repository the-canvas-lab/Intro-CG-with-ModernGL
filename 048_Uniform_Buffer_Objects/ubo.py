...
#
# Scene: four cubes, alternating between two shader programs (warm diffuse /
# cool toon). Camera matrices and the animated light live in two UBOs that
...
class Scene:
    def __init__(self):
        ...
        self.prog_warm = self.ctx.program(
            vertex_shader=vert, fragment_shader=load_shader('shaders/warm.frag'))
        self.prog_cool = self.ctx.program(
            vertex_shader=vert, fragment_shader=load_shader('shaders/cool.frag'))

        # The mesh has pos(3f) normal(3f) uv(2f); these shaders ignore the
        # UVs, so '2x4' skips those 8 bytes of stride.
        vbo = self.ctx.buffer(load_obj_mesh('../models/cube.obj'))
        self.vaos = [
            self.ctx.vertex_array(
                prog, [(vbo, '3f 3f 2x4', 'in_position', 'in_normal')])
            for prog in (self.prog_warm, self.prog_cool)
        ]
    ...
    def render(self, t):
        ...
        for i, x in enumerate(self.cube_xs):
            ...
            prog  = (self.prog_warm, self.prog_cool)[i % 2]
