...
# =============================================================================
# Replaces the uniform material colors from lesson 032 with two textures:
#
#   Diffuse map  (container2.png)          — per-fragment surface color
#   Specular map (container2_specular.png) — per-fragment highlight intensity
#
# The wooden frame of the container absorbs specular light (dark in the
# specular map) while the metal edges reflect it brightly (white in the
# specular map).  Watch the highlight as the lamp orbits — it only appears
...
pygame.display.set_caption("034 — Lighting Maps")
...
def load_texture(ctx, path):
    ...
    # OpenGL origin is bottom-left; pygame origin is top-left — flip vertically.
...
def load_obj_mesh(path):
    """Return interleaved [position(3), normal(3), uv(2)] per vertex."""
    base = os.path.dirname(os.path.abspath(__file__))
...
class Scene:
    def __init__(self):
        ...
        # Buffer layout: position(3f) + normal(3f) + uv(2f) = 32 bytes/vertex
        ...
        # Lamp only needs position; skip the 20 bytes of normal + uv.
        ...
        # Tell the shader which texture unit holds each map.
